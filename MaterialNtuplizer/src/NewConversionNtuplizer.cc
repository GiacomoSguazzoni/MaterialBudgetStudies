...
// $Id: NewConversionNtuplizer.cc,v 1.2 2010/05/06 13:02:30 cerati Exp $
//
//
...
private:
  ...
  void fillR2S(TrackRef *tk, bool valid_pvtx, reco::Vertex &the_pvtx, KinematicVertex& vtx );
  void fillR2S_refit(Track **tk, bool valid_pvtx, reco::Vertex &the_pvtx );
  void fillR2SHitPattern(TrackRef* tk);
  ...
  int getBeforeHit(TrackRef& tk, float recoPhoR, float vtx_z);
  ...
  GlobalVector getStateAtVertex(TrackRef& tk, float recoPhoR,float vtx_z,float& iphi);
  ...
  void AssociateByKineConstraints(vector<PhotonMCTruth>& mcPhotons,KinematicVertex& vtx,TrackRef *arrayTK);
...
NewConversionNtuplizer::beginJob()
{
  ...
  ntupleS2R->Branch("x",		&(s2rbranch.mcPhoton.x),	       	"x/F");
  ntupleS2R->Branch("y",		&(s2rbranch.mcPhoton.y),		"y/F");
  ntupleS2R->Branch("z",		&(s2rbranch.mcPhoton.z),		"z/F");
  ...
  ntupleR2S->Branch("x",		&(r2sbranch.recoPhoton.x),		"x/F");
  ntupleR2S->Branch("y",		&(r2sbranch.recoPhoton.y),		"y/F");
  ntupleR2S->Branch("z",		&(r2sbranch.recoPhoton.z),		"z/F");
...
void NewConversionNtuplizer::fillR2S(TrackRef *tk, bool valid_pvtx, reco::Vertex &the_pvtx, KinematicVertex& vtx ){
  for(size_t i=0;i<2;i++){
...
int NewConversionNtuplizer::getBeforeHit(TrackRef& tk, float recoPhoR, float vtx_z){
...
fillMCinS2R(const PhotonMCTruth& Pho,std::vector<ElectronMCTruth>& electrons){
  ...
  s2rbranch.mcPhoton.x     = Pho.vertex().x();
  s2rbranch.mcPhoton.y     = Pho.vertex().y();
  s2rbranch.mcPhoton.z     = Pho.vertex().z();
}
...
getStateAtVertex(TrackRef& tk, float recoPhoR,float vtx_z,float& iphi){
  ...
  std::cout << " getStateAtVertex check tk " << tk->innerMomentum() << std::endl; 
  ...
  std::cout << " getStateAtVertex check  TSOS " << myTSOS.globalMomentum() << std::endl; 
...
getConversionVertexPairs(const ConversionCollection* convColl){
  if (prints) cout << "loop 1, converision size=" << convColl->size() << endl;
...
void NewConversionNtuplizer::
AssociateByKineConstraints(vector<PhotonMCTruth>& mcPhotons,KinematicVertex& vtx, TrackRef *arrayTK){
  double simPhoR(0), simPhoZ(0), simPhoEta(0), simPhoPt(0), simPhoPhi(0), simPhoTheta(0);
...
AssociateByHit(const edm::Event& iEvent,const Conversion& conv){
  ...
  tc.push_back(RefToBase<Track>(conv.tracks().front()));
  tc.push_back(RefToBase<Track>(conv.tracks().back ()));
  ...
  std::vector<std::pair<TrackingParticleRef, double> > tp1 = p[tc[0]];
  std::vector<std::pair<TrackingParticleRef, double> > tp2 = p[tc[1]];
...
void NewConversionNtuplizer::fillS2R(const Conversion& conv,KinematicVertex& vtx,const PhotonMCTruth& Pho){ 
  ...
  TrackRef tk[2] = {conv.tracks().front(),conv.tracks().back()};
  math::XYZVector photonMom = tk[0]->momentum()+tk[1]->momentum();
